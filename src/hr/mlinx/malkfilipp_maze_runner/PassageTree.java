package hr.mlinx.malkfilipp_maze_runner;
...
import static hr.mlinx.malkfilipp_maze_runner.Cell.Type.PASSAGE;
...
public class PassageTree {
    ...
    private record Edge(int firstCell, int secondCell) {};
    ...
    private List<Cell> createPassages(List<Edge> spanningTree) {
        return spanningTree
                .stream()
                .map(edge -> {
                    var first = fromIndex(edge.firstCell());
                    var second = fromIndex(edge.secondCell());
